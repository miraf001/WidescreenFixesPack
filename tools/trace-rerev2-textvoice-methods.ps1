...
param(
    ...
    [ValidateRange(1, 256)]
    [int]$MaximumEvents = 48,
    [ValidateRange(1, 32)]
    [int]$SamplesPerMethod = 6
)
...
try {
    ...
    & $python.Source (Join-Path $PSScriptRoot 'trace-rerev2-textvoice-methods.py') `
        --pid $game[0].Id `
        --timeout-seconds $TimeoutSeconds `
        --maximum-events $MaximumEvents `
        --samples-per-method $SamplesPerMethod
    exit $LASTEXITCODE
}
finally {
